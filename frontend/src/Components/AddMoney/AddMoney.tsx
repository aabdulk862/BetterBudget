...
export const AddMoney: React.FC = () => {
  ...
  const handleForm = async (e: React.FormEvent<HTMLFormElement>) => {
    e.preventDefault();
    if (
      envs.reduce((acc, env) => acc + parseInt(env.amount), 0) ==
        parseInt(amount) &&
      parseInt(amount) > 0
    ) {
      ...
      .then((data) => {
        ...
        for (let i = 0; i < data.length; i++) {
          ...
          if (
            ...
            data[i].user.userId === user.userId
          ) {
            temp.push({
  ...
  useEffect(() => {
    ...
    for (let i = 0; i < envs.length; i++) {
      if (
        parseInt(envs[i].amount) > 0 &&
        parseInt(envs[i].amount) > envs[i].max_limit - envs[i].balance
      ) {
        setEnableBtn(true);
        setError(
          `Error: Amount for ${envs[i].envelope_description} exceeds available envelope limit`,
        );
        return;
      }
    }
    ...
    <Box className="main-add-money-container">
      <Box className="add-money-container">
        ...
        <Typography
          ...
          gutterBottom
        >
          Add incoming money information below, then allocate amount to{" "}
          {user.role === "ROLE_MANAGER" ? "all" : "your"} envelopes as needed.
        </Typography>
